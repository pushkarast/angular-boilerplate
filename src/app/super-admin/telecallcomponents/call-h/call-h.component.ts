...
@Component({
  selector: 'app-call-h',
  templateUrl: './call-h.component.html',
  styleUrls: ['./call-h.component.css']
})
export class CallHComponent implements OnInit {
    ...
    this._AdminService
      ...
      .then((data) => {
        if (data?.status) {
          ...
          this.feedbackData.get('dialId')?.setValue(data?.data?.call_id)
        }
        else {
